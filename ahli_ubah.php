<?php
    $row = $db->get_row("SELECT * FROM tb_ahli WHERE kode_ahli='$_GET[ID]'"); 
?>
...
<form method="post" action="?m=ahli_ubah&ID=<?=$row->kode_ahli?>&kode_alt=<?=$row->kode_alt?>">
<div class="form-group">
    <label>Alternatif</label>
    <select disabled="" class="form-control" name="kode_alt"><?=get_data_alt_option($row->kode_alt)?></select>
</div>
<div class="form-group">
    <label>Nama Ahli</label>
    <input class="form-control" type="text" name="nama_alt" value="<?=$row->nama_ahli?>" />
</div>
<td>Posisi</td>
<td>
	<div class="form-group">
