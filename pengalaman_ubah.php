<?php
    $row = $db->get_row("SELECT * FROM tb_pengalaman WHERE kode_pengalaman='$_GET[ID]'"); 
?>
<div class="page-header">
    <h1>Ubah Data Pengalaman</h1>
</div>
<div class="row">
<div class="col-sm-6">
<?php if($_POST) include'aksi.php'?>
<form method="post" action="?m=pengalaman_ubah&ID=<?=$row->kode_pengalaman?>&kode_ahli=<?=$row->kode_ahli?>">
<div class="form-group">
    <label>Pengalaman</label>
    <select disabled="" class="form-control" name="kode_ahli"><?=get_ahli_option($row->kode_ahli)?></select>
</div>
<div class="form-group">
    <label>mulai</label>
